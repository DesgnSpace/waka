...
- When a single reverse proxy terminates TLS and either sets `X-Real-IP` or appends the client to `X-Forwarded-For`, set `TRUST_PROXY=true` so per-IP rate limits key on the real client address. Waka prefers `X-Real-IP` and otherwise reads the rightmost `X-Forwarded-For` entry, the one the trusted proxy appended. Without it, all proxied clients share one limit bucket.
