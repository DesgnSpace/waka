...
import { generateApiKey, getUserApiKeys, deleteApiKey } from "@/lib/api-keys";
...
const updateApiKeySchema = z
  ...
  .refine((d) => d.permissions !== undefined || d.expiresAt !== undefined || d.expires_at !== undefined || d.rateLimitPerMinute !== undefined || d.dailySendLimit !== undefined, {
...
export async function createApiKey(req: Req): Promise<Response> {
  ...
  const rateLimitPerMinute = parsed.rateLimitPerMinute;
  const dailySendLimit = parsed.dailySendLimit;
...
export async function updateApiKey(req: Req): Promise<Response> {
  ...
  const updates: { permissions?: string[] | null; expiresAt?: string | null; rateLimitPerMinute?: number | null; dailySendLimit?: number | null } = {};
  if (parsed.permissions !== undefined) updates.permissions = parsed.permissions;
  if (expiresAt !== undefined) updates.expiresAt = expiresAt as string | null;
  if (parsed.rateLimitPerMinute !== undefined) updates.rateLimitPerMinute = parsed.rateLimitPerMinute;
  if (parsed.dailySendLimit !== undefined) updates.dailySendLimit = parsed.dailySendLimit;
  const { updateApiKey: updateApiKeyRecord } = await import("@/lib/api-keys");
  await updateApiKeyRecord(pathUuid(req), user.id, updates);
...
export async function emailLogs(req: Req): Promise<Response> {
  ...
  const allowedStatus = ["pending", "sent", "failed", "delivered", "bounced", "complained", "scheduled", "sending"] as const;
