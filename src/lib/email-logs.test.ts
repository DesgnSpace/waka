...
const { emailLogs } = await import("@/server/handlers");
...
test("no filters adds no extra clauses", async () => {
  ...
  expect(countSql).not.toContain("el.status");
  expect(countSql).not.toContain("coalesce(el.to_emails");
...
test("recipient filter adds LIKE clause with parameter", async () => {
  ...
  expect(sql).toContain("coalesce(el.to_emails::text, '') || ' ' || coalesce(el.cc_emails::text, '') || ' ' || coalesce(el.bcc_emails::text, '')) LIKE");
...
test("a scheduled row's payload never reaches the logs response", async () => {
  const req = new Request("http://localhost/api/emails/logs?recipient=to@example.com", {
    headers: { authorization: `Bearer wka_test_${"a".repeat(32)}` },
  }) as Req;
  req.params = {};

  const res = await emailLogs(req);

  expect(res.status).toBe(200);
  const body = await res.json();
  expect(body.data.emails).toHaveLength(1);
  expect(body.data.emails[0]).not.toHaveProperty("payload");
});
